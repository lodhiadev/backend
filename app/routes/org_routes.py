...
async def create_org(payload: OrgCreate):
    ...
    existing = master_db.organizations.find_one({"organization_name": org_name})
    if existing:
...
async def get_org(organization_name: str):
    org = master_db.organizations.find_one({"organization_name": organization_name}, {"_id":0})
    if not org:
...
async def update_org(payload: OrgCreate):
    ...
    master_db.organizations.update_one({"organization_name": org_name}, {"$set": {
